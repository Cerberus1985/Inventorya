...
FROM `nuke_stories` `t1`, `nuke_authors` `t2`, `nuke_stories_cat` `t3`, `nuke_topics` `t4`
    ...
    echo "<h3>ADODB Version: $ADODB_vers";
    echo "<p>Host: <i>$db->host</i>";
    echo "<br>Database: <i>$db->database</i>";
    echo "<br>PHP: <i>$phpv $ext</i></h3>";

    flush();

    echo 'Current timezone: '.date_default_timezone_get().'<p>';
...
//	$db->Execute('select * from table where val=? AND value=?', array('val'=>'http ://www.whatever.com/test?=21', 'value'=>'blabl'));
 ...
 //////////////////////////////////////////////////////////////////////////////////////////
    ...
    if ($val1[0] == 50 and count($val1) == 1 and $val2[0] == 50 and count($val2) == 1) {
    ...
    echo '<p>FETCH_MODE = ASSOC: Should get 1, Caroline ASSOC_CASE='.ADODB_ASSOC_CASE.'</p>';
 ...
 //	$db->debug=true;
    ...
    if ($rs) {
        $arr = $rs->GetArray(10);
        if (count($arr) != 10 || trim($arr[1][1]) != 'John' || trim($arr[1][2]) != 'Lim') {
    ...
    for ($loop = 0; $loop < 1; $loop++) {
        ...
        $rs = $db->CacheExecute(4, 'select distinct firstname,lastname from ADOXYZ');
        if ($rs) {
            echo ' Multiple, Alan and George selected: '.$rs->GetMenu('menu', ['Alan', 'George'], false, true);
            if (empty($rs->connection)) {
                print '<b>Connection object missing from recordset</b></br>';
            }
        } else {
